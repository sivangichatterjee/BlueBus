...
public class PerBusPdf {
	private List<PerbusQuery> queryResult;
	private Date d1;
	private Date d2;

	public PerBusPdf(List<PerbusQuery> queryResult, Date d1, Date d2) {
	...
	private void writeTableHeader(PdfPTable table) {
		...
		cell.setPhrase(new Phrase("Per Bus Ticket Booked", font));
		table.addCell(cell);
	...
	public void export(HttpServletResponse response) throws DocumentException, IOException {
		...
		Paragraph p = new Paragraph("Per Bus Ticket Booking Data from " + d1.toString() + " to " + d2.toString(), font);
